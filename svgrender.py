	...
	if __debug__:
		...
		def check_dom_events(self, handler):
			...
			if handler == "motion_notify_event":
				...
				assert any(_ms_ev.type_ == "mouseleave" for _ms_ev in self.emitted_dom_events) if (nup and pnup and nup[-1] != pnup[-1] and self.ancestors(pnup[-1]) - self.ancestors(nup[-1])) else True, "For a `motion_notify_event`, when `previous_nodes_under_pointer` and `nodes_under_pointer` aren't empty and top element is different and top `previous_nodes_under_pointer` have all top `nodes_under_pointer` ancestors, a DOM event 'mouseleave` should be emitted"
				...
				assert any(_ms_ev.type_ == "mouseenter" for _ms_ev in self.emitted_dom_events) if (nup and pnup and nup[-1] != pnup[-1] and self.ancestors(nup[-1]) - self.ancestors(pnup[-1])) else True, "For a `motion_notify_event`, when `previous_nodes_under_pointer` and `nodes_under_pointer` aren't empty and top element is different and top `previous_nodes_under_pointer` have all top `nodes_under_pointer` ancestors, a DOM event 'mouseenter` should be emitted"
