...
class SVGFormat:
	...
	def draw_image(self, view, document, ctx, box):
		...
		if not self.is_svg_document(document):
			return NotImplemented
		print("draw image", view, document, ctx, box)
		self.__render_xml(view, ctx, box, document.getroot(), [document], self.get_document_url(document), 0, None)
	
	def pointer_event(self, view, document, box, pointer):
		if not self.is_svg_document(document):
			return NotImplemented
