...
class Client:
	def __init__(self, guipath='gui.glade', csspath='style.css'):
		...
		for n, child in enumerate(box_main.get_children()):
			print(gtk.Buildable.get_name(child))
			box_main.remove(child)
			stack_main.add_named(child, gtk.Buildable.get_name(child))
		main_parent = box_main.get_parent()
		#pos = box_main.props.position
		main_parent.remove(box_main)
		main_parent.pack_start(stack_main, True, True, 0)
		main_parent.reorder_child(stack_main, 1)
		stack_main.show()
		stack_main.set_visible_child_name('box_login')
		...
		self['scrolledwindow_roster'].add(repeat_roster)
		repeat_roster.show_all()
		
		self.roster_ui = repeat_roster
	...
	def setup_login(self):
		print("setup_login")
		self.stack_main.set_visible_child_name('box_login')
		print("(setup_login end)")
	
	def setup_roster(self):
		print("setup_roster")
		self.stack_main.set_visible_child_name('scrolledwindow_roster')
		print("(setup_roster end)")
	...
	def roster_update(self, roster):
		print("roster_update")
		for item in roster.get_payload()[0].getiterator():
			print(item)
		pass
	...
	def session_start(self, ignore):
		...
		self.setup_roster()
		self.__client.send_presence(pstatus="Laying around, gathering dust.", ppriority=0)
		self.__client.get_roster()
