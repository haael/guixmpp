...
class XMPPClient:
				...
				try:
					while True:
						...
						if (stanza := await wait_for(self.recv_stanza(), end_timeout)) in (None, Ellipsis):
							if stanza == None:
								logger.info(" ...got end tag.")
							elif stanza == Ellipsis:
								logger.info(" ...timeout waiting for end tag.")
							else:
								logger.warning("???")
							break
						else:
	...
	async def recv_stanza(self):
		"Receive raw stanza from the stream. Spawn a read task, then return the stanza if received, None if server closed connection gracefully or Ellipsis if read task was cancelled."
			...
			async with TaskGroup() as task_group:
				...
				while self.running:
					...
					if stanza == None:
						...
						logger.debug(f" Interrupt message: {repr(self.interrupt_message)}")
	...
	async def on_presence(self, stanza):
		...
		for child in stanza:
			if child.tag == '{jabber:client}show': # FIXME:
				try:
					show = child.text.strip()
				except (TypeError, AttributeError):
					show = None
			elif child.tag == '{jabber:client}status': # FIXME:
				try:
					status = child.text.strip()
				except (TypeError, AttributeError):
					status = None
			elif child.tag == '{jabber:client}priority': # FIXME:
				try:
					priority = int(child.text.strip())
				except (TypeError, ValueError):
					priority = 0
			else:
				logger.warning(f"Unexpected element in presence stanza: {child.tag}, {tostring(child).decode('utf-8')}")
		
		return 'presence', id_, from_, priority, show, status
	 
	async def on_message(self, stanza): # FIXME
		id_ = stanza.attrib.get('id', None)
		from_ = stanza.attrib.get('from', None)
		body = []
	...
	async def iq_get(to, *body, timeout=10):
		return await self.query('get', to, *body, timeout=timeout)
	
	async def iq_set(to, *body, timeout=10):
		...
		async with XMPPClient('haael@dw.live/discovery') as client:
			...
			async for stanza in client:
				...
				if event == 'ready':
					...
					@client.handle
					async def get_roster(client, expect):
						roster, = await client.iq_get(None, fromstring(b'<query xmlns="jabber:iq:roster"/>'))
						for item in roster:
					...
					@client.handle
					async def register(client, expect):
						reg_query, = await client.iq_get(None, fromstring(b'<query xmlns="jabber:iq:register"/>'))
					...
					@client.handle
					async def boom_get(client, expect, type_=type_, id_=id_, from_=from_, elements=elements):
						try:
					...
					@client.handle
					async def boom_set(client, expect, id_=id_, from_=from_):
